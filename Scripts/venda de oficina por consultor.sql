SELECT
	gu.NOME ,
	SUM(fmc.TOT_NOTA_FISCAL)-sum(fmc.VALDESCONTO)-sum(fmc.VALDESCONTO_MO) AS TOTAL_LIQUIDO,
	sum(fmc.TOT_MERCADORIA),
	sum(fmc.TOT_SERVICOS),
	sum(fmc.VALDESCONTO),
	sum(fmc.VALDESCONTO_MO)
FROM
...
WHERE
	...
	AND fmc.EMPRESA = 1
	AND fmc.REVENDA = 1
	AND fmc.TIPO_TRANSACAO in ('O21','G21','O26','O31')
